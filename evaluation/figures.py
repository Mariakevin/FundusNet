...
def plot_reliability_diagram(probs, labels, save_path=None, n_bins=15,
    ...
    if len(bin_centers) > 0:
        ...
        ax.bar(bin_centers, bin_accs, width=bar_width, alpha=0.7,
               color="#2196F3", label="Accuracy", edgecolor="white", linewidth=0.5)
        ax.plot(bin_confs, bin_accs, "o-", color="#FF5722", lw=2, markersize=6,
    ...
    ax.text(0.98, 0.02, f"ECE = {ece:.4f}", transform=ax.transAxes,
...
def plot_accuracy_refusal(refusal_rates, accuracies, save_path=None,
                           title="Accuracy-Refusal Tradeoff",
                           xlabel="Refusal Rate", ylabel="Accuracy (Accepted)"):
    ...
    ax.plot(refusal_rates, accuracies, "o-", color="#2196F3", lw=2.5, markersize=8,
            label="Selective Ensemble")
    ax.fill_between(refusal_rates, accuracies, alpha=0.1, color="#2196F3")

    if len(refusal_rates) > 1:
        baseline_acc = accuracies[0]
        ax.axhline(y=baseline_acc, color="#9E9E9E", linestyle="--", lw=1.5,
                    label=f"Full ensemble ({baseline_acc:.3f})")
...
def plot_ablation_bars(ablation_summary, save_path=None,
    ...
    ax.text(0.02, 0.98, "← Worse | Better →", transform=ax.transAxes,
...
def plot_class_distribution(labels, categories, save_path=None,
...
def plot_gradcam_grid(images, predictions, ground_truths=None, save_path=None,
...
def plot_uncertainty_analysis(uncertainties, is_correct, save_path=None,
                               title="Uncertainty vs. Prediction Correctness"):
    ...
    ax1.hist(correct_unc, bins=30, alpha=0.7, color="#4CAF50",
             label=f"Correct (n={len(correct_unc)})", density=True, edgecolor="white")
    ax1.hist(incorrect_unc, bins=30, alpha=0.7, color="#F44336",
             label=f"Incorrect (n={len(incorrect_unc)})", density=True, edgecolor="white")
    ax1.set(xlabel="Uncertainty (Entropy)", ylabel="Density",
            title="Uncertainty Distribution by Correctness")
    ax1.legend(frameon=True)
    ...
    ax2.set(xlabel="Uncertainty (smoothed)", ylabel="Accuracy (smoothed)",
            title="Accuracy vs. Uncertainty")
    ax2.set_ylim([0, 1.05])
    ax2.grid(True, alpha=0.3)
